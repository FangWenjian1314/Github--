...
<!--  status-icon属性为输入框添加了表示校验结果的反馈图标-->
  <el-form :inline="true" :model="form" status-icon :rules="rules" ref="form" style="width: 80%" label-width="100px">
    ...
    <el-form-item label="描述" prop="description">
      <el-input type="textarea" style="width: 400px;" :autosize="{ minRows: 4, maxRows: 10}" v-model="form.description" placeholder="请输入描述"></el-input>
    </el-form-item>
    ...
    <el-form-item label="借书积分" prop="score">
      <el-input-number v-model="form.score" :min="10" :max="30" label="积分"></el-input-number>
    </el-form-item>
  ...
  data() {
    const checkNums = (rule, value, callback) => {
      ...
      if (value < 0 || value > 1000) {
        callback(new Error("请输入大于0小于等于1000的整数"))
      }
      callback()
    };
    return {
